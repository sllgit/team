...
class UserModel extends Model{
    ...
    public function createToken($user){
        ...
        $token->addClaim(new Claim\Issuer('sunlonglong'));
        $token->addClaim(new Claim\JwtId('1'));
        $token->addClaim(new Claim\NotBefore(new \DateTime('now')));
        $token->addClaim(new Claim\Subject('http://www.1810b.com'));
    ...
    public function verifyToken($token=""){
        ...
        $context->setIssuer('sunlonglong');
        $context->setSubject('http://www.1810b.com');
        ...
        $user = json_decode(decrypt($user,config("userToken.key"),config("userToken.iv")),true);
        // 返回用户信息
        return $this->getUserInfoByUid(base_decode($user['id'])) ?? null;
    }
